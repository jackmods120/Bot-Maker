...
# ══════════════════════════════════════════════════════════════════════════════
# ── یارمەتیدەرەکان (سیکوریتی و ئەدمین)
# ══════════════════════════════════════════════════════════════════════════════
...
# ██  کیبۆردەکان
# ══════════════════════════════════════════════════════════════════════════════
def kb_main(uid: int, is_admin_user: bool, notif_status: bool = True) -> ReplyKeyboardMarkup:
    notif_btn = "🔔 ئاگادارکردنەوە: چالاک" if notif_status else "🔕 ئاگادارکردنەوە: ناچالاک"
    if is_admin_user:
        return ReplyKeyboardMarkup([[KeyboardButton("➕ دروستکردنی بۆتی نوێ"),  KeyboardButton("📂 بۆتەکانم")],[KeyboardButton("👑 پانێلی سەرەکی"),         KeyboardButton("📊 ئامارەکان")],[KeyboardButton(notif_btn)]
...
def kb_control(is_admin_user: bool) -> ReplyKeyboardMarkup:
...
KB_OWNER_MAIN = ReplyKeyboardMarkup([
    [KeyboardButton("👥 بەشی بەکارهێنەران"),   KeyboardButton("🤖 بەشی بۆتەکان")],[KeyboardButton("📨 بەشی پەیام"),           KeyboardButton("💎 بەشی VIP")],[KeyboardButton("🛡 بەشی ئەمنیەت"),         KeyboardButton("📢 بەشی کانال")],[KeyboardButton("⚙️ بەشی سیستەم"),          KeyboardButton("📊 ئامارەکان")],[KeyboardButton("🔙 گەڕانەوە بۆ سەرەتا")],
], resize_keyboard=True)

KB_USERS = ReplyKeyboardMarkup([[KeyboardButton("👥 لیستی هەموو بەکارهێنەران"), KeyboardButton("🔍 گەڕان بۆ بەکارهێنەر")],[KeyboardButton("📋 زانیاری بەکارهێنەر بە ID"), KeyboardButton("📊 ئامارەکانی بەکارهێنەران")],[KeyboardButton("🗑 سڕینەوەی بەکارهێنەر"),     KeyboardButton("🔙 گەڕانەوە بۆ پانێلی سەرەکی")],
], resize_keyboard=True)

KB_BOTS = ReplyKeyboardMarkup([[KeyboardButton("🤖 لیستی هەموو بۆتەکان"),   KeyboardButton("🟢 بۆتە چالاکەکان")],[KeyboardButton("🔴 بۆتە ڕاگیراوەکان"),       KeyboardButton("📊 ئامارەکانی بۆتەکان")],[KeyboardButton("▶️ دەستپێکردنی هەموو"),       KeyboardButton("⏸ وەستاندنی هەموو")],[KeyboardButton("🗑 سڕینەوەی بۆت بە ID"),      KeyboardButton("🔍 گەڕان بۆ بۆت")],
    [KeyboardButton("🔙 گەڕانەوە بۆ پانێلی سەرەکی")],
], resize_keyboard=True)

KB_MSG = ReplyKeyboardMarkup([[KeyboardButton("📨 بڵاوکردنەوە بۆ هەموو"),    KeyboardButton("📨 بڵاوکردنەوە بۆ VIP")],[KeyboardButton("📨 بڵاوکردنەوە بۆ نا-VIP"),   KeyboardButton("📬 پەیام بۆ بەکارهێنەرێک")],[KeyboardButton("📌 دانانی پەیامی سیستەم"),     KeyboardButton("🗑 سڕینەوەی پەیامی سیستەم")],
    [KeyboardButton("🔙 گەڕانەوە بۆ پانێلی سەرەکی")],
], resize_keyboard=True)

KB_VIP = ReplyKeyboardMarkup([[KeyboardButton("💎 لیستی VIPەکان"),            KeyboardButton("➕ زیادکردنی VIP")],[KeyboardButton("➖ لابردنی VIP"),               KeyboardButton("📊 ئامارەکانی VIP")],[KeyboardButton("💎 VIP بۆ هەمیشەیی"),         KeyboardButton("🔍 پشکنینی VIP بەکارهێنەر")],[KeyboardButton("🔙 گەڕانەوە بۆ پانێلی سەرەکی")],
], resize_keyboard=True)

KB_SEC = ReplyKeyboardMarkup([[KeyboardButton("🚫 بلۆک کردنی بەکارهێنەر"),    KeyboardButton("✅ لابردنی بلۆک")],[KeyboardButton("📋 لیستی بلۆکەکان"),            KeyboardButton("⚠️ ئاگادارکردنەوەی بەکارهێنەر")],[KeyboardButton("🔙 گەڕانەوە بۆ پانێلی سەرەکی")],
], resize_keyboard=True)

KB_CHAN = ReplyKeyboardMarkup([[KeyboardButton("➕ زیادکردنی کانالی داواکراو"), KeyboardButton("➖ لابردنی کانالی داواکراو")],[KeyboardButton("📋 لیستی کانالەکان"),           KeyboardButton("✅ چالاککردنی داواکردن")],[KeyboardButton("❌ لەکارخستنی داواکردن"),      KeyboardButton("🔙 گەڕانەوە بۆ پانێلی سەرەکی")],
], resize_keyboard=True)

KB_SYS = ReplyKeyboardMarkup([
    [KeyboardButton("➕ زیادکردنی ئەدمین"),          KeyboardButton("➖ لابردنی ئەدمین")],[KeyboardButton("📋 لیستی ئەدمینەکان"),          KeyboardButton("⚙️ زانیاری سیستەم")],[KeyboardButton("🔄 نوێکردنەوەی هەموو وەبهووک"), KeyboardButton("🖼 گۆڕینی وێنەی بەخێرهاتن")],
    [KeyboardButton("🔙 گەڕانەوە بۆ پانێلی سەرەکی")],
], resize_keyboard=True)
...
async def master_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(txt, parse_mode="HTML", reply_markup=kb_main(uid, is_adm, notif_status))

# ══════════════════════════════════════════════════════════════════════════════
# ██  هەندڵەری پەیامەکان (تێکست و وێنە)
# ══════════════════════════════════════════════════════════════════════════════
async def handle_all_messages(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_blocked(uid): return

    # Check Force Join (گەر لە ناوەڕاستدا دەرچوو بێت)
    joined, missing = await check_force_join(uid, ctx.bot)
    if not joined:
        btn = [[InlineKeyboardButton(f"📢 کەناڵی {ch}", url=f"https://t.me/{ch}")] for ch in missing]
    ...
    # ئەگەر وێنەی ناردبێت بۆ گۆڕینی وێنەی بەخێرهاتن
    if update.message.photo:
        if state == "change_photo_url" and is_adm:
            photo_id = update.message.photo[-1].file_id
            await db_put("system/photo_id", photo_id)
            await db_del(f"users/{uid}/state")
    ...
    if txt == "🔙 گەڕانەوە بۆ لیست":
        await db_del(f"users/{uid}/state")
        await show_bot_list(update, uid, is_adm, notif_status)
        return
    ...
    if "ئاگادارکردنەوە:" in txt:
        new_status = not notif_status
        await db_patch(f"users/{uid}", {"notifications": new_status})
        st_text = "چالاک کرا ✅" if new_status else "ناچالاک کرا 🔕"
        await update.message.reply_text(f"زەنگەکانی بۆت {st_text}.", reply_markup=kb_main(uid, is_adm, new_status))
        return

    if txt == "➕ دروستکردنی بۆتی نوێ":
        kb = ReplyKeyboardMarkup([["🍓 بۆتی ڕیاکشن"],["🔙 گەڕانەوە بۆ سەرەتا"]], resize_keyboard=True)
        await update.message.reply_text("🤖 <b>جۆری بۆت هەڵبژێرە:</b>", parse_mode="HTML", reply_markup=kb)
        return

    if txt == "🍓 بۆتی ڕیاکشن":
        await db_put(f"users/{uid}/state", "await_token")
        kb = ReplyKeyboardMarkup([["🔙 گەڕانەوە بۆ سەرەتا"]], resize_keyboard=True)
        await update.message.reply_text("🍓 <b>تۆکێنی بۆتەکەت لێرە بنێرە:</b>", parse_mode="HTML", reply_markup=kb)
        return
    ...
    if txt in CTRL:
        await handle_control(update, uid, txt, is_adm)
        return

    if txt in ("📊 ئامارەکان", "📊 ئامارەکانم"):
        await show_stats(update, uid, is_adm, notif_status)
        return

    # ════ پانێلی ئەدمین ════
    if is_adm:
        if txt == "👑 پانێلی سەرەکی":
            await show_owner_main(update); return

        if txt == "👥 بەشی بەکارهێنەران": await update.message.reply_text("هەڵبژێرە:", reply_markup=KB_USERS); return
        if txt == "🤖 بەشی بۆتەکان": await update.message.reply_text("هەڵبژێرە:", reply_markup=KB_BOTS); return
        if txt == "📨 بەشی پەیام": await update.message.reply_text("هەڵبژێرە:", reply_markup=KB_MSG); return
        if txt == "💎 بەشی VIP": await update.message.reply_text("هەڵبژێرە:", reply_markup=KB_VIP); return
        if txt == "🛡 بەشی ئەمنیەت": await update.message.reply_text("هەڵبژێرە:", reply_markup=KB_SEC); return
        if txt == "📢 بەشی کانال": await update.message.reply_text("هەڵبژێرە:", reply_markup=KB_CHAN); return
        if txt == "⚙️ بەشی سیستەم": await update.message.reply_text("هەڵبژێرە:", reply_markup=KB_SYS); return

        # System / Admins
        if txt == "➕ زیادکردنی ئەدمین":
            await db_put(f"users/{uid}/state", "add_admin")
            await update.message.reply_text("🆔 ئایدی کەسەکە بنووسە:", reply_markup=ReplyKeyboardMarkup([["❌ هەڵوەشاندنەوە"]], resize_keyboard=True)); return
        if txt == "➖ لابردنی ئەدمین":
            await db_put(f"users/{uid}/state", "del_admin")
            await update.message.reply_text("🆔 ئایدی کەسەکە بنووسە:", reply_markup=ReplyKeyboardMarkup([["❌ هەڵوەشاندنەوە"]], resize_keyboard=True)); return
        if txt == "📋 لیستی ئەدمینەکان":
            admins = await db_get("system/admins") or {}
            msg = "📋 <b>لیستی ئەدمینەکان:</b>\n" + "\n".join([f"• <code>{k}</code>" for k in admins.keys()]) if admins else "هیچ ئەدمینێک نییە جگە لە خاوەن."
        ...
        if txt == "🖼 گۆڕینی وێنەی بەخێرهاتن":
            ...
            await update.message.reply_text("🖼 <b>تکایە وێنەکە ڕاستەوخۆ لێرە بنێرە (وەک وێنە نەک فایل یان لینک):</b>", parse_mode="HTML", reply_markup=kb)
            return
        ...
        if txt == "➕ زیادکردنی کانالی داواکراو":
            ...
            await update.message.reply_text("یوزەرنەیم بنووسە بێ @:", reply_markup=ReplyKeyboardMarkup([["❌ هەڵوەشاندنەوە"]], resize_keyboard=True)); return
        ...
        if txt == "✅ چالاککردنی داواکردن":
            await db_put("system/force_join", True)
            await update.message.reply_text("✅ چالاک کرا."); return
        if txt == "❌ لەکارخستنی داواکردن":
            await db_put("system/force_join", False)
            await update.message.reply_text("❌ لەکار خرا."); return

        # Broadcasts
        if txt == "📨 بڵاوکردنەوە بۆ هەموو":
            await db_put(f"users/{uid}/state", "bc_all")
            await update.message.reply_text("نامەکەت بنووسە:", reply_markup=ReplyKeyboardMarkup([["❌ هەڵوەشاندنەوە"]], resize_keyboard=True)); return

    await handle_states(update, uid, txt, state, is_adm, notif_status)

# ══════════════════════════════════════════════════════════════════════════════
# ── فەنکشنەکانی خێرایی (asyncio.gather) و نیشاندان
# ══════════════════════════════════════════════════════════════════════════════
async def show_owner_main(update: Update):
    # خێرایی موشەک بە هۆی هێنانی داتا لە یەک کاتدا!
...
async def show_bot_control(update: Update, uid: int, bid: str, info: dict, is_adm: bool):
    ...
    msg = (
        ...
        "━━━━━━━━━━━━━━━━━━━\n"
        f"🤖 ناو : {html.escape(info.get('bot_name','ناسناو'))}\n"
        f"🔗 یوزەر : @{info.get('bot_username','ناسناو')}\n"
        f"📊 دۆخ : {st}\n"
        "━━━━━━━━━━━━━━━━━━━"
    )
...
async def handle_control(update: Update, uid: int, txt: str, is_adm: bool):
...
async def owner_sys_info(update: Update):
    ...
    msg = (
        ...
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"🌐 URL : <code>{PROJECT_URL}</code>\n"
        f"👥 بەکارهێنەران : <b>{len(all_u)}</b>\n"
        f"🤖 بۆتەکان : <b>{len(all_b)}</b>\n"
        f"✅ جۆینی ناچاری : <b>{'چالاک' if fj else 'لەکارخراو'}</b> ({len(req_chs)} کەناڵ)"
    )
...
async def process_child_update(token: str, body: dict):
    try:
        ...
        sys_photo = await db_get("system/photo_id") or await db_get("system/photo_url") or PHOTO_URL
        ...
        async with httpx.AsyncClient(timeout=10) as c:
            if txt.startswith("/start"):
                ...
                # ئەگەر photo_id بێت یان url هەردووکیان هەرکاردەکات
